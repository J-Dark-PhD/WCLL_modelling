...
def shifted_cmap_temp(data):
    orig_cmap = cm.inferno
    shifted_cmap = shiftedColorMap(
        orig_cmap,
        midpoint=abs(0 - data.min()) / abs(data.max() - data.min()),
    )
...
def get_data(T_range, bz_flow_range):
    ...
    for T in T_range:
        ...
        for mass_flow_bz in bz_flow_range:
...
def h_coeff_bz(mass_flow_rate):
    T_standard = 585
    vel = para_flow_velocity_bz(mass_flow_rate)
    h = para_h_bz(T=T_standard, u=vel)
    return h
...
(
    ...
    structure_max_temperature,
    bz_pipes_max_temperature,
) = get_data(temp_range, mass_flow_range_bz)
...
X, Y = np.meshgrid(mass_flow_range_bz, temp_range)
...
plt.scatter(0.85491, 585, color="white", marker="*", s=10)
plt.colorbar(CS, label=r"Maximum temperature (K)", format="%.0f")
ax.set_xlabel(r"BZ mass flow rate (kg s$^{-1}$)")
ax.set_ylabel(r"Coolant bulk temperature (K)")
ax_1_ticks = ax.get_xticks()[:-1]
ax2 = ax.twiny()
h_min, h_max = (
    h_coeff_bz(ax.get_xlim()[0]),
    h_coeff_bz(ax.get_xlim()[1]),
)
ax2.set_xlim(h_min, h_max)
ax2.set_xlabel(r"Heat transfer coefficient at 585K (W m$^{-2}$ K$^{-1}$)")
plt.tight_layout()
...
ax.clabel(CS3, inline=True, fontsize=10, fmt="%.0f")
plt.scatter(0.85491, 585, color="white", marker="*", s=10)
plt.colorbar(CS, label=r"Maximum temperture (K)", format="%.0f")
ax.set_xlabel(r"BZ mass flow rate (kg s$^{-1}$)")
ax.set_ylabel(r"Coolant bulk temperature (K)")
ax_1_ticks = ax.get_xticks()[:-1]
ax2 = ax.twiny()
...
plt.scatter(0.85491, 585, color="black", marker="*", s=10)
...
plt.scatter(0.85491, 585, color="black", marker="x", s=10)
# plt.scatter(X, Y, color="black", marker="x", s=9)
plt.colorbar(CS, label=r"Average surface temperture difference (\%)", format="%.1f")
plt.scatter(0.85491, 585, color="black", marker="*", s=10)
ax.set_xlabel(r"BZ mass flow rate (kg s$^{-1}$)")
