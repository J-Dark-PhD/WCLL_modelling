...
S_0_values = [2.0e21, 3.9e21, 7.5e21, 1.5e22, 2.8e22, 5.5e22, 1.1e23, 2.1e23, 4.0e23]
S_0_ref = 1.427e23
k_B = 8.6e-5
...
folder = "../../parametric_studies/varying_lipb_solubility/"
...
total = (
    np.array(y_pipes)
    + np.array(y_structure)
    + np.array(y_first_wall)
    + np.array(y_lipb)
)
x_annotation = solubilities_normalised[-1] * 1.15

plt.plot(
    solubilities_normalised, y_pipes, label="BZ pipes", color=grey_eurofer, marker="+"
)
plt.plot(
...
plt.plot(
    solubilities_normalised, y_first_wall, label="First wall", color=red_W, marker="+"
)
plt.plot(solubilities_normalised, y_lipb, label="LiPb", color=green_lipb, marker="+")
# plt.plot(
#     solubilities_normalised, total,
#     label='Total', color='black', marker="+")

plt.annotate("BZ pipes", (x_annotation, y_pipes[-1] * 1), color=grey_eurofer)
plt.annotate("Structure", (x_annotation, y_structure[-1] * 0.9), color=grey_eurofer)
plt.annotate("First wall", (x_annotation, y_first_wall[-1] * 1.1), color=red_W)
...
xlabel = "Normalised LiPb solubility" + r" $K_S$(600)/$K_{S-SC}$(600)"
plt.xlabel(xlabel)
