...
S_0_values = np.logspace(np.log10(2e21), np.log10(4e23), num=0)

# taken from (Reiter, 1990)
T_reiter = np.arange(508, 700, step=1)

S_0_lipb_reiter_H = 4.4310e20  # (atom/m3 Pa^-0.5)
E_S_lipb_reiter_H = 0.01399  # (eV)

S_0_lipb_reiter_D = 4.2857e20  # (atom/m3 Pa^-0.5)
E_S_lipb_reiter_D = 0.01399  # (eV)

S_0_lipb_reiter_T = 4.2131e20  # (atom/m3 Pa^-0.5)
...
if __name__ == "__main__":
    ...
    plt.plot(1000 / T_aiello, S_lipb_aillo, label="H (Aiello'08)", color="darkred")
    plt.annotate("H (Aiello'08)", (1.6, 1.5e22), color="darkred")

    plt.plot(
        1000 / T_schumacher, S_lipb_schumacher, label="H (Schumacher'90)", color="peru"
    )
    plt.annotate("H (Schumacher'90)", (1.2, 8e21), color="peru")

    plt.plot(
        1000 / T_alberro, S_lipb_alberro, label="T (Alberro'15)", color="forestgreen"
    )
    plt.annotate("T (Alberro'15))", (1.3, 3.5e21), color="forestgreen")

    plt.plot(1000 / T_chan, S_lipb_chan, label="H (Chan'84)", color="teal")
    plt.annotate("H (Chan'84)", (1.35, 1.2e21), color="teal")

    plt.plot(1000 / T_reiter, S_lipb_reiter_T, label="T (Reiter'90)", color="purple")
    ...
    plt.plot(
        1000 / T,
        S_eurofer_chen(T),
        label="Eurofer (Chen'21)",
        color="dimgrey",
        linestyle="dashed",
    )
    plt.plot(
        1000 / T, S_eurofer_esteban(T), label="Eurofer (Esteban'07)", color="dimgrey"
    )
    plt.plot(
        1000 / T, S_eurofer_leblond(T), label="Eurofer (Leblond'21)", color="black"
    )

    for S_fun, T in zip(S_functions, T_exps):
        plt.plot(1000 / T, S_fun, color="green", alpha=0.4)
    ...
    for S_0 in S_0_values:
        plt.plot(1000 / T, S(T, S_0, E_S_lipb_test), color="red", label="Test values")
