...
for S_0 in S_0_values:
    ...
    pipes_inv_dict[S_0] = sum(
        [data["Total_retention_volume_{}".format(i)] for i in range(10, 21)]
    )
    pipes_flux_dict[S_0] = sum(
        [data["Flux_surface_{}_solute".format(i)] * -1 for i in range(28, 39)]
    )
    structure_dict[S_0] = data["Total_retention_volume_7"]
    baffle_dict[S_0] = data["Total_retention_volume_8"]
    FW_channels_dict[S_0] = (
...
cb = plt.colorbar(
    sm, ax=axs, label=r"$K_{S_\mathrm{LiPb}}$(600 K) (m$^{-3}$ Pa$^{-0.5}$)"
)
# plt.tight_layout()
plt.show()
