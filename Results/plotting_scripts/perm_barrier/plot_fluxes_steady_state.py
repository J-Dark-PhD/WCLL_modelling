...
S_0_log_values = [2.4e23, 2.4e22, 2.4e21, 2.4e20, 2.4e19, 2.4e18, 2.4e17, 2.4e16]
...
for S_0_log in S_0_log_values:
    filename = folder + "/S_0_eur={:.1e}/derived_quantities.csv".format(S_0_log)
    data = np.genfromtxt(filename, delimiter=",", names=True)
    y_fw_cooling_log.append(
        sum([data["Flux_surface_{}_solute".format(i)] * -1 for i in range(24, 28)])
    )
    y_BZ_cooling_log.append(
...
plt.plot(
    solubilities_log_normalised,
    y_fw_cooling_log / y_fw_cooling_log[0],
    label="FW cooling channel",
    color=grey_eurofer,
    marker="+",
)
plt.plot(
    solubilities_log_normalised,
    y_BZ_cooling_log / y_BZ_cooling_log[0],
    label="BZ Cooling",
    color=green_lipb,
    marker="+",
)
...
plt.annotate(
    "FW cooling \n channel",
    (x_annotation, (y_fw_cooling_log / y_fw_cooling_log[0])[-1] * 0.6),
    color=grey_eurofer,
)
plt.annotate(
    "BZ Cooling",
    (x_annotation, (y_BZ_cooling_log / y_BZ_cooling_log[0])[-1] * 0.6),
    color=green_lipb,
)
...
for S_0 in S_0_values:
    ...
    y_BZ_cooling.append(
        sum([data["Flux_surface_{}_solute".format(i)] * -1 for i in range(40, 49)])
    )
...
plt.annotate(
    "FW cooling \n channel",
    (x_annotation, (y_fw_cooling / y_fw_cooling[0])[-1]),
    color=grey_eurofer,
)
plt.annotate(
    "BZ Cooling",
    (x_annotation, (y_BZ_cooling / y_BZ_cooling[0])[-1] * 0.8),
    color=green_lipb,
)
