...
for S_0_log in S_0_log_values:
    ...
    y_eurofer_log.append(
        sum([data["Total_retention_volume_{}".format(i)] for i in range(10, 20)])
        + data["Total_retention_volume_8"]
...
plt.plot(
    solubilities_log_normalised,
    y_first_wall_log / y_first_wall_log[0],
    label="First wall",
    color=red_W,
    marker="+",
)
...
plt.annotate(
    "Eurofer",
    (x_annotation, (y_eurofer_log / y_eurofer_log[0])[-1]),
    color=grey_eurofer,
)
plt.annotate(
    "Tungsten",
    (x_annotation, (y_first_wall_log / y_first_wall_log[0])[-1] * 0.6),
    color=red_W,
)
plt.annotate(
    "LiPb", (x_annotation, (y_lipb_log / y_lipb_log[0])[-1] * 1.2), color=green_lipb
)
