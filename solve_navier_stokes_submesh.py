...
def NS_solver(temp):
    ...
    mesh_sub = Mesh()
    print("loading submesh")
    XDMFFile("3D_Results/sub_mesh.xdmf").read(mesh_sub)
    print("loaded submesh")

    volume_markers_sub = MeshFunction("size_t", mesh_sub, mesh_sub.topology().dim(), 1)
    surface_markers_sub = MeshFunction(
        "size_t", mesh_sub, mesh_sub.topology().dim() - 1, 0
    )
    ...
    sym_boundary = DirichletBC(
        W.sub(0).sub(2), Constant(0.0), surface_markers_sub, id_sym
    )
    ...
    mesh_temp = "3D_Results/T_sub_mesh.xdmf"
    T = Function(V_CG1)
    ...
    solve(
        ...
        bcu,
        solver_parameters={
            "newton_solver": {"linear_solver": "gmres", "preconditioner": "jacobi"}
        },
    )
