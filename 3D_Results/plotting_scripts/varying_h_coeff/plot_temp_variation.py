...
for temp in temp_range:
    ...
    T_pipes_data = np.mean(
        [data["Average_T_volume_{}".format(i)] for i in range(10, 21)]
    )
    T_pipes.append(T_pipes_data)
    T_structure.append(data["Average_T_volume_7"])
    T_baffle.append(data["Average_T_volume_8"])
    T_lipb.append(data["Average_T_volume_6"])
    T_W.append(data["Average_T_volume_9"])
